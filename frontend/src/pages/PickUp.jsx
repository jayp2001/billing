...
const PickUp = () => {
  ...
  const getData = async () => {
    await axios.get(`${BACKEND_BASE_URL}menuItemrouter/getItemData?menuId=base_2001`, config)
      .then((res) => {
        setData(res.data);
      })
      .catch((error) => {
  ...
  const addBillData = async () => {
    ...
    console.log("DISCOUNT", billData.subTotal - billData.settledAmount, billData.subTotal, billData.settledAmount)
    await axios.post(`${BACKEND_BASE_URL}billingrouter/addPickUpBillData`, customData, config)
      .then((res) => {
        setSuccess(true);
        setLoading(false);
        setItems([]);
        ...
        // const htmlString = renderToString(<KOT data={res.data} />)
  ...
  const saveBill = () => {
    if (loading || success) {
      ...
      if (!items || items.length < 1 || !billData || !billData.subTotal || !billData.settledAmount || !billData.discountType || (billData.discountType != 'none' && !billData.discountValue)) {
        setError(
  ...
  const handleUnitChange = (e) => {
    const value = e.target.value;
    const options = fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList ? fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList : [];
  ...
  const handleEnterPressFirst = (e) => {
    ...
    if (e.key === 'Enter') {
      e.preventDefault();
      const matchingProduct = data.find(item => item.itemCode.toString() === value || item.itemShortKey.toString().toLocaleLowerCase() === value.toString().toLocaleLowerCase());
      ...
      } else if (value === matchingProduct.itemCode.toString() || matchingProduct.itemShortKey.toString().toLocaleLowerCase() === value.toString().toLocaleLowerCase()) {
        e.preventDefault();
        setValidationError(false)
        setFullFormData(prevState => ({
          ...prevState,
  ...
  const handleEnterPressSecond = (e) => {
    if (e.key === 'Enter') {
      setFullFormData((perv) => ({
        ...perv,
        unit: fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList ? fullFormData.selectedItem.variantsList[0].unit : '',
        itemPrice: fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList ? fullFormData.selectedItem.variantsList[0].price : 0,
        selectedUnit: fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList ? fullFormData.selectedItem.variantsList[0] : {},
        price: (fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList ? fullFormData.selectedItem.variantsList[0].price : 0) * fullFormData.qty
      }))
      console.log('name', fullFormData.inputName);
      e.preventDefault();
      if (fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList ? fullFormData.selectedItem.variantsList.length > 1 ? true : false : false) {
        unitInputRef.current && unitInputRef.current.focus();
      } else if (fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList ? fullFormData.selectedItem.variantsList.length == 1 ? true : false : false) {
        commentInputRef.current && commentInputRef.current.focus();
      }
    }
  };
  ...
  const handleAddItem = (e) => {
    if (e.key === 'Enter') {
      if (fullFormData.inputName !== '') {
        const isExist = items.findIndex((item) => item.inputCode == fullFormData.inputCode && item.unit == fullFormData.unit)
        ...
        if (isExist != -1) {
          ...
          setItems(prevItems => prevItems?.map((data, index) => (
            isExist == index ? { ...data, qty: parseFloat(data.qty) + parseFloat(fullFormData.qty), price: data.price + (fullFormData.qty * fullFormData.itemPrice) } : data
          )));
          setFullFormData({
            inputCode: '',
            inputName: '',
  ...
  const handleIncreaseQuantity = (index) => {
    setItems(prevItems => {
      ...
      updatedItems[index].price = updatedItems[index].qty * updatedItems[index].itemPrice;
      return updatedItems;
    });
    setBillData((perv) => ({
      ...perv,
      subTotal: billData.subTotal + items[index].itemPrice,
      settledAmount: (billData.subTotal + items[index].itemPrice) - (billData.discountType == 'fixed' ? billData.discountValue : billData.discountType == 'percentage' ? (billData.subTotal + items[index].itemPrice) * (billData.discountValue / 100) : 0),
    }))
  };
  ...
  const handleDecreaseQuantity = (index, qty1) => {
    ...
    setBillData((perv) => ({
      ...perv,
      subTotal: (qty1) > 1 ? (billData.subTotal - items[index].itemPrice) : billData.subTotal,
      settledAmount: (qty1) > 1 ? (billData.subTotal - items[index].itemPrice) - (billData.discountType == 'fixed' ? billData.discountValue : billData.discountType == 'percentage' ? (billData.subTotal - items[index].itemPrice) * (billData.discountValue / 100) : 0) : billData.subTotal - (billData.discountType == 'fixed' ? billData.discountValue : billData.discountType == 'percentage' ? billData.subTotal * (billData.discountValue / 100) : 0),
    }))
  };
  if (loading) {
    console.log('>>>>??')
        ...
        <div className="right_top_header gap-6 p-2 flex">
          ...
          <div className="w-80">
            <Autocomplete
              options={data ? data : []}
              defaultValue={null}
              getOptionLabel={(options) => options.itemName ? options.itemName : ''}
          ...
          <div className="w-28">
            ...
            <FormControl fullWidth>
              <NativeSelect
                ...
                placeholder='Units'
                onBlur={(e) => {
                  const json = fullFormData.selectedItem.variantsList ? fullFormData.selectedItem.variantsList?.filter((varient) => varient.unit == e.target.value) : {};
                  setFullFormData((perv) => ({
                    ...perv,
                    selectedUnit: json[0],
                    itemPrice: json[0] && json[0].price ? json[0].price : '',
                    unit: e.target.value,
                    price: (json[0] && json[0].price ? json[0].price : 0) * fullFormData.qty
                  }))
                }}
                onChange={(e) => {
                  const json = fullFormData.selectedItem.variantsList ? fullFormData.selectedItem.variantsList?.filter((varient) => varient.unit == e.target.value) : {};
              ...
              >
                {
                  fullFormData && fullFormData.selectedItem && fullFormData.selectedItem.variantsList && fullFormData.selectedItem.variantsList?.map((data, index) => (
          ...
          <div className="w-12">
            <button onClick={handleReset} className="button mt-1 text-sm px-2 py-1 rounded-sm text-white">RESET</button>
          </div>
        </div>
      </section>
            ...
            <div className="left_bill_menu text-base w-full h-full">
              ...
              <div className=" p-0 text-base ">
                <div className="bg-gray-200">
                  <div className="grid grid-cols-12 w-full p-2">
                    ...
                    <div className="col-span-1 justify-self-end pr-3">Price</div>
                    <div className="col-span-1 justify-self-end pr-3">Total</div>
                  </div>
                </div>
                <div className={validationError ? 'main_billError' : 'main_bill1'}>
                  {
                    items.length === 0 && (
                  ...
                  {items.map((item, index) => (
                    <div key={index} className="bg-amber-50 billin_content itemBorder p-2 text-lg">
                      <div className="grid grid-cols-12 content-center gap-2">
                        ...
                        <div className="col-span-3 justify-self-start">{item.inputName.itemName}</div>
                        <div className="col-span-3 justify-self-center">{item.comment}</div>
                        <div className="col-span-2 justify-self-start">
                          <div className="flex h-full align-star main_div">
                            <div className="plus_button">
                              <button onClick={() => handleDecreaseQuantity(index, item.qty)} className='border quantity_button p-0'>-</button>
                            </div>
                            <input type="text" value={item.qty} className='w-8 text-center' readOnly />
                            <div className="plus_button">
                ...
                <div className="w-full p-2 h-full bg-gray-700 text-white">
                  <div className="flex w-full justify-between pl-2 pr-2 gap-4 main_div">
                    <div>
                      ...
                      >
                        <div>
                          <FormControlLabel value="cash" control={<Radio name='radio'
                            sx={{
                              color: '#fff',
                              '&.Mui-checked': {
                                color: '#fff',
                        ...
                        <div>
                          <FormControlLabel value="due" control={<Radio name='radio'
                            sx={{
                              color: '#fff',
                              '&.Mui-checked': {
                                color: '#fff',
                        ...
                        <div>
                          <FormControlLabel value="online" control={<Radio name='radio'
                            sx={{
                              color: '#fff',
                              '&.Mui-checked': {
                                color: '#fff',
                        ...
                        <div>
                          <FormControlLabel value="complementary" control={<Radio name='radio'
                            sx={{
                              color: '#fff',
                              '&.Mui-checked': {
                                color: '#fff',
              ...
              <div className='w-full text-base flex justify-between pl-4 pr-3 text-white gap-4 bg-gray-500'>
                <div>
                  ...
                  >
                    <div>
                      <FormControlLabel value="none" control={<Radio name='radio' sx={{
                        color: '#fff',
                        '&.Mui-checked': {
                          color: '#fff',
                        },
                    ...
                    <div>
                      <FormControlLabel value="percentage" control={<Radio name='radio' sx={{
                        color: '#fff',
                        '&.Mui-checked': {
                          color: '#fff',
                        },
                    ...
                    <div>
                      <FormControlLabel value="fixed" control={<Radio name='radio' sx={{
                        color: '#fff',
                        '&.Mui-checked': {
                          color: '#fff',
                        },
                    ...
                    <TextField
                      ...
                      onChange={(e) => {
                        setBillData((perv) => ({
                          ...
                          settledAmount: billData.discountType == 'fixed' ? billData.subTotal - e.target.value : billData.subTotal * (1 - e.target.value / 100),
                        }))
                      }}
                      id="discountValue"
                      variant="outlined"
                      ...
                      // label="Discount"
                      InputProps={{
                        endAdornment: billData.discountType == 'percentage' ? <InputAdornment position="end"><PercentIcon fontSize='small' /></InputAdornment> : billData.discountType == 'fixed' ? <InputAdornment position="end"><CurrencyRupeeIcon fontSize='small' /></InputAdornment> : <InputAdornment><BlockIcon fontSize='small' /></InputAdornment>,
                ...
                <div className='discountPadding flex gap-3' style={{ width: '200px', textAlign: 'end' }}>
                  <div className='text-white text-xl mt-1 w-full'>{'Total Discount : ' + (billData.subTotal - billData.settledAmount)}</div>
                </div>
              </div>
              ...
              <div className="w-full text-base flex justify-end pl-4 pb-1 pr-3 text-white gap-4 bg-gray-700">
                <div className='discountPadding flex gap-3'>
                  <div className={`text-white text-xl mt-1 `}>{'Settled Amount   : '}</div>
                  <TextField
                    className='sarchTextTEST'
                    // onChange={(e) => { onSearchChange(e); debounceFunction() }}
                    ...
                    InputProps={{
                      ...
                      style: { fontSize: 16, background: '#fff', color: billData.settledAmount <= 0 && billData.subTotal > 0 ? 'red' : 'black' }
                    }}
                    // InputLabelProps={{ style: { fontSize: 16 } }}
                    fullWidth
                  />
              ...
              <div className="w-full text-base flex justify-center gap-4 p-1 mt-1 ">
                <div><button className='text-base button save_button py-1 rounded-md text-white' onClick={() => {
                  saveBill()
                }}>Save</button></div>
